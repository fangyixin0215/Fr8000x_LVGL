...
static void index_change(lv_obj_t * obj, int32_t index)
{
    ...
    //lv_img_set_src(obj, animimg->dsc[idx]);
	  if(animimg_flag!=1)return ;
		if ((index_change_flag == 1) && (idx == 0))
    {
        lv_img_set_src(obj, animimg->dsc[animimg->pic_count-1]);
        index_change_flag = 0;
    }
    else
    {
...
void lv_stop_animimg(void) //返回显示状态
{
	 animimg_flag=0;
}
#endif
