...
#ifndef FR_UART_PRIVATE_H
#define FR_UART_PRIVATE_H
 ...
 *  a legitimate fr_device structure.
 * SOURCE
 */
 ...
 *  bitfield.
 * EXAMPLE
 *  uint32_t irqID = FR_BIT_GET(in32(UART_IIR), UART_IIR_INTERRUPT_ID);
 * NOTES
...
/****id* uart.macros/FR_CC_DEFINE_UART_PARAMS
 * DESCRIPTION
 *  This macro is used to initialize a fr_uart_param structure.  To use
 ...
 * SOURCE
 */
#define FR_CC_DEFINE_UART_PARAMS_3_01(prefix) { \
...
#define FR_CC_DEFINE_UART_PARAMS_2_00(prefix) { \
 ...
 *  APB_DATA_WIDTH at compile time will force all FR_apb_uart memory map
 ...
 *  device is checked in the fr_device data structure.
 * SOURCE
 */
// Byte accesses remain unchanged regardless of APB data width
#define UART_IN8P   FR_IN8P
#define UART_OUT8P  FR_OUT8P

// Change 32-bit data accesses depending on APB data width

#define UART_INP    FR_IN32_32P
#define UART_OUTP   FR_OUT32_32P

/*****/
#define UART_FIFO_DEPTH_0     32
#define UART_FIFO_DEPTH_1     16
...
do {                                                                \
    ...
    while(FR_BIT_GET(reg, UART_LSR_DR) == Fr_set) {                 \
        instance->rxHold >>= 8;                                     \
        instance->rxHold |= (FR_IN8P(portmap->rbr_thr_dll) << 24);  \
        if(--instance->rxIdx == 0) {                                \
...
do {                                                                \
    ...
    if((fr_uart_isPtimeEnabled(uart_addr) == true) &&               \
            (instance->fifos_enabled == true)) {                    \
        /* While the Tx FIFO is not full (PTIME enabled). */        \
        while(FR_BIT_GET(reg, UART_LSR_THRE) == Fr_clear) {         \
            if(instance->txIdx == 0) {                              \
                instance->txHold = *(instance->txBuffer++);         \
                instance->txIdx = 4;                                \
            }                                                       \
            FR_OUT8P((instance->txHold & 0xff),                     \
                    portmap->rbr_thr_dll);                          \
            instance->txHold >>= 8;                                 \
            instance->txIdx--;                                      \
            if(--instance->txRemain == 0)                           \
    ...
    /* Fill up the FIFO if the THRE bit is Set (Tx FIFO empty) */   \
        ...
        if(FR_BIT_GET(reg, UART_LSR_THRE) == Fr_set){               \
            if(uart_addr == UART0){                                 \
                UART_X_FIFO_WRITE(UART_FIFO_DEPTH_0);               \
            }                                                       \
            else{                                                   \
                UART_X_FIFO_WRITE(UART_FIFO_DEPTH_1);               \
            }                                                       \
        }                                                           \
    }                                                               \
    /* Can only write one character if FIFOs are disabled */        \
    else if(FR_BIT_GET(reg, UART_LSR_THRE) == Fr_set) {             \
        if(instance->txIdx == 0) {                                  \
            instance->txHold = *(instance->txBuffer++);             \
            instance->txIdx = 4;                                    \
        }                                                           \
        FR_OUT8P((instance->txHold & 0xff), portmap->rbr_thr_dll);  \
        instance->txHold >>= 8;                                     \
        instance->txIdx--;                                          \
        instance->txRemain--;                                       \
    }                                                               \
 ...
 * DESCRIPTION
 *  This macro disables FR_apb_uart interrupts, to avoid shared data
 ...
 * SEE ALSO
 *  UART_EXIT_CRITICAL_SECTION, fr_uart_instance
 * SOURCE
 */
 ...
 * DESCRIPTION
 *  This macro restores FR_apb_uart interrupts, after a critical code
 *  section.  It uses the saved IER value in the instance structure to
 *  accomplish this.
 * SEE ALSO
 *  UART_ENTER_CRITICAL_SECTION, fr_uart_instance
 * SOURCE
 */
...
enum fr_uart_state {
    ...
    Uart_state_rx_req,      // user must call fr_uart_receive()
    Uart_state_tx_rx,       // driver is transceiving data
    Uart_state_tx_rx_req    // ditto as above; the user must call
                            // fr_uart_receive() to handle Rx transfer
};
/*****/

/****is* uart.data/fr_uart_param
 * DESCRIPTION
 ...
 *  the correct values and be pointed to by the (struct fr_device).cc
 *  member of the relevant uart device structure.
 * SOURCE
 */
struct fr_uart_param {
    bool afce_mode;         // automatic flow control
    bool dma_extra;         // FR_ahb_dmac compatibility signals
...
/****is* uart.data/fr_uart_portmap
 * DESCRIPTION
 *  This is the structure used for accessing the uart register memory
 *  map.
 * EXAMPLE
 *  struct fr_uart_portmap *portmap;
 *  portmap = (struct fr_uart_portmap *) FR_APB_UART_BASE;
 *  lineStatus = INP(portmap->lsr);
 * SOURCE
 */
struct fr_uart_portmap {
    ...
    //for rs485 interface
    volatile uint32_t tcr;          // transceiver control        (0xac)
    volatile uint32_t deen;         // driver output enable       (0xb0)
    volatile uint32_t reen;         // receiver output enable     (0xb4)
    volatile uint32_t det;          // driver output enable timing(0xb8)
    volatile uint32_t tat;          // turnaround timing register (0xbc)
    // fractional baud divisor en == 1
    volatile uint32_t dlf;          // divisor latch fraction     (0xc0)
    // uart 9bit data en == 1
    volatile uint32_t rar;          // receive address            (0xc4)
    volatile uint32_t tar;          // transmit address           (0xc8)
    volatile uint32_t lcrext;       // line extended control      (0xcc)
    
    volatile uint32_t reserved2[9];// reserved                (0xd0-0xf0)
 ...
 * SOURCE
 */
struct fr_uart_instance {
    enum fr_uart_state state;           // uart driver state
    ...
    fr_callback listener;          	// user event listener
    fr_callback txCallback;        	// user Tx callback function
    fr_callback rxCallback;        	// user Rx callback function
    enum fr_uart_callback_mode callbackMode;    // Rx callback mode
    ...
    enum fr_uart_tx_trigger txTrigger;  // saved user Tx trigger
    #ifdef CFG_DMA
    struct fr_dma_config dmaTx;         // DMA Tx configuration
    struct fr_dma_config dmaRx;         // DMA Rx configuration
    #endif
};
/*****/

/****if* uart.functions/fr_uart_resetInstance
 * DESCRIPTION
 *  Initializes the uart instance structure members.
 * ARGUMENTS
 *  dev         -- FR_apb_uart device handle
 * RETURN VALUE
 *  none
 * NOTES
 ...
 * SOURCE
 */
void fr_uart_resetInstance(uint32_t uart_addr);
/*****/

/****if* uart.functions/fr_uart_flushRxFifo
 * DESCRIPTION
 ...
 * SOURCE
 */
int fr_uart_flushRxFifo(uint32_t uart_addr);
/*****/

/****if* uart.functions/fr_uart_flushRxHold
 * DESCRIPTION
 ...
 * SOURCE
 */
void fr_uart_flushRxHold(uint32_t uart_addr);
/*****/

/****if* uart.functions/fr_uart_autoCompParams
 * DESCRIPTION
 ...
 * SOURCE
 */
int fr_uart_autoCompParams(uint32_t uart_addr);
/*****/
